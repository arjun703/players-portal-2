...
export default function Dashboard() {
  ...
  useEffect(() => {
    async function fetchDashboard() {
        try {
            const response = await fetch('/api/club-dashboard'); // Adjust the API endpoint URL as needed
            if (!response.ok) {
            ...
            setPlayersInClub(data.playersInClub)
        } catch (error) {
            alert( error.message)
        }finally{
          setIsLoading(false)
              ...
              <>
                ...
                <div className="avatar-name justify-content-between d-flex align-items-center" >
                  ...
                  <div style={{textAlign:'center', marginTop: '-15px'}}>
                    <h1>Welcome, {club?.name.split(' ')[0]}</h1>
                  </div>
                  <div style={{display:'flex', justifyContent:'center', margin:'30px'}}>
                    <Link href={'/edit-club-profile/'}>
                      <Button>
                        Edit Profile
                      </Button>
                    </Link>
                ...
                <div style={{padding:'30px'}}>
                    {
                      playersInClub.length ? <PlayersInClub playersInClub={playersInClub}/> : <NoPlayersInClub />
                    }
                </div>
              </>
            )
...
function NoPlayersInClub(){
  return(
    <div style={{textAlign:'center'}}>
      <div>
        The players in your club will appear here.
      </div>
    </div>
  )
}
...
function PlayersInClub({playersInClub}){
  ...
  return(
    <>
      <div>
        <h3 style={{textAlign:'center'}}>Club Members</h3>
      </div>
      <Grid container spacing={2}>
        {playersInClub.map((tp, index) => (
